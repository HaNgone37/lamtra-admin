...
import { sizeService } from '@/services/productService'
...
export default function SizeModal({
  ...
  const validateForm = () => {
    if (!formData.name.trim()) {
      onError('Vui lòng nhập tên kích thước')
      return false
    }
    if (!formData.additionalprice || parseFloat(formData.additionalprice) < 0) {
      onError('Giá thêm phải là số không âm')
      return false
    }
    return true
  }
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      onError(isEdit ? 'Lỗi khi cập nhật kích thước. Vui lòng thử lại.' : 'Lỗi khi tạo kích thước. Vui lòng thử lại.')
    } finally {
      setLoading(false)
    }
  }
      ...
      >
        ...
        >
          <h2 style={{ fontSize: '20px', fontWeight: 'bold', color: '#2B3674', margin: 0 }}>
            {isEdit ? 'Chỉnh Sửa Kích Thước' : 'Thêm Kích Thước'}
          </h2>
          <button
            onClick={onClose}
            style={{
          ...
          <div style={{ display: 'flex', flexDirection: 'column', gap: '16px' }}>
            ...
            <div>
              <label style={{ display: 'block', fontSize: '14px', fontWeight: '600', color: '#2B3674', marginBottom: '8px' }}>
                Tên kích thước<span style={{ color: '#F56565' }}>*</span>
              </label>
              <input
                type="text"
                name="name"
