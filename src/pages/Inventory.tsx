...
import { ClipboardList, Box, FlaskConical } from "lucide-react";
...
export default function Inventory() {
  ...
  const userRole = (
    localStorage.getItem("userRole") || "branch manager"
  ).toLowerCase();
  ...
  const [activeTab, setActiveTab] = useState<
    "categories" | "inventory" | "recipes"
  >("categories");
  const [toastMessages, setToastMessages] = useState<ToastMessage[]>([]);
  const [isRestockModalOpen, setIsRestockModalOpen] = useState(false);
  const [selectedIngredientId, setSelectedIngredientId] = useState<string>("");
  const [selectedIngredient, setSelectedIngredient] =
    useState<BranchInventory | null>(null);
  ...
  const [selectedIngredientForAudit, setSelectedIngredientForAudit] =
    useState<string>("");
  ...
  const [isAddInventoryItemModalOpen, setIsAddInventoryItemModalOpen] =
    useState(false);
  const [availableIngredients, setAvailableIngredients] = useState<
    Ingredient[]
  >([]);
  const [isLoadingAvailableIngredients, setIsLoadingAvailableIngredients] =
    useState(false);
    ...
    try {
      ...
      if (isSuperAdmin) {
        ...
        if (userBranchId) {
          ...
          if (branchData) {
            setBranches([
              { ...branchData, branchid: String(branchData.branchid) },
            ]);
            setSelectedBranch(userBranchId);
  ...
  const loadBranchInventory = async (branchId: string) => {
    ...
    try {
      ...
      const data =
        await branchInventoryService.getBranchInventoryByBranch(branchId);
  ...
  const loadRecipes = async (productId: string, sizeId?: string) => {
    ...
    try {
      console.log(
        "[Inventory] Loading recipes for productId:",
        productId,
        "sizeId:",
        sizeId,
      );
  ...
  const handleAudit = async () => {
    ...
    try {
      ...
      console.log("[AUDIT] Starting with values:", {
        branchId,
        systemstock,
        physicalstock,
        employeeid,
      ...
      if (result.success) {
        ...
        await Promise.all([loadBranchInventory(selectedBranchId)]);
      }
    } catch (error) {
  ...
  const handleAddIngredientToProduct = async () => {
    if (
      !selectedProduct ||
      !newIngredientForm.ingredientid ||
      !newIngredientForm.amount
    ) {
      showToast("Vui lòng điền đầy đủ thông tin", "error");
      return;
    }
  ...
  const loadAvailableIngredients = async (branchId: string) => {
    try {
      ...
      console.log(
        "[AVAILABLE] Loading available ingredients for branch:",
        branchId,
      );
      const available =
        await branchInventoryService.getAvailableIngredientsForBranch(branchId);
  ...
  const handleAddInventoryItem = async (ingredientId: string | number) => {
    try {
      ...
      await branchInventoryService.addToInventory(
        Number(branchId),
        ingredientId,
        0,
      );
    ...
    <div style={{ padding: "20px" }}>
      ...
      <AuditModal
        ...
        selectedIngredientName={
          ingredients.find(
            (i) =>
              String(i.ingredientid) === String(selectedIngredientForAudit),
          )?.name || ""
        }
      ...
      {activeTab === "inventory" && (
        <InventoryStockTab
          ...
          onRestockClick={(ingredientId: string) => {
            ...
            const ingredient = branchInventory.find(
          ...
          onAuditClick={(ingredientId: string, currentStock: number) => {
            ...
            setAuditForm({
              actualStock: String(currentStock),
              reason: "Hao hụt",
            });
