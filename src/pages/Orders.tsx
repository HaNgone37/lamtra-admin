...
export const Orders: React.FC = () => {
  ...
  const branchDisplay = userBranchId ? branches.find(b => b.branchid === userBranchId)?.name || 'Chi nhánh' : 'Chi nhánh'
        ...
        for (const detail of details) {
          ...
          if (toppingIds.length > 0) {
            ...
            const toppingInfoMap = new Map(
              toppingInfo?.map(t => [t.toppingid, { name: t.name, price: t.price }]) || []
            )
            ...
            <table className="w-full">
              <thead>
                <tr style={{ borderBottom: '1px solid #E0E5F2' }}>
                  <th className="text-left py-3 px-4 text-sm font-semibold" style={{ color: '#2B3674' }}>
                    Mã đơn
                  </th>
                  <th className="text-left py-3 px-4 text-sm font-semibold" style={{ color: '#2B3674' }}>
                    Khách hàng
              ...
              {isSuperAdmin && (
                ...
                >
                  <div className="flex items-start gap-2 mb-3">
                    <AlertCircle size={16} style={{ color: '#4318FF', marginTop: '2px' }} />
                    <p className="text-xs font-semibold" style={{ color: '#2B3674' }}>
                    ...
                    <div>
                      ...
                      <div className="relative">
                        <select
                          ...
                          style={{ backgroundColor: '#FFFFFF', color: '#2B3674', border: '1px solid #E0E5F2' }}
                        >
                          {STATUS_OPTIONS.map(status => (
                            <option key={status} value={status}>
                              {status}
      ...
      {showStatusModal && selectedOrder && (
        <div className="fixed inset-0 bg-black/50 z-50 flex items-center justify-center p-4">
          <div className="bg-white rounded-xl shadow-2xl max-w-sm w-full">
            ...
            <div className="p-6 space-y-4">
              <div className="bg-blue-50 p-4 rounded-lg">
                ...
                <p className="text-sm text-gray-700 mt-2">
                  ...
                  <span style={{ color: '#8F9CB8' }}>{selectedOrder.status}</span>
                </p>
                <p className="text-sm text-gray-700 mt-2">
                  <strong>Trạng thái mới:</strong>{' '}
                  <span style={{ color: '#4318FF', fontWeight: 'bold' }}>{newStatus}</span>
                </p>
              </div>
            ...
            {/* Actions */}
            <div className="flex gap-3 p-6 border-t" style={{ borderColor: '#E0E5F2' }}>
              <button
                onClick={() => setShowStatusModal(false)}
                disabled={updatingStatus}
