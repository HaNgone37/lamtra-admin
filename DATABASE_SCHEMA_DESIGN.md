...
7.  **`orders` (Đơn hàng):** `orderid` (varchar, PK), `totalamount`, `discountamount`, `shippingfee`, `finalamount`, `paymentmethod`, `ordertype`, `status` ('Chờ'/'Đang làm'/'Xong'/'Hủy'), `orderdate`, `branchid` (FK), `customerid` (FK, Nullable).
8.  **`orderdetails` (Món trong đơn):** `orderdetailid` (int8, PK), `orderid` (FK), `productid` (FK), `sizeid` (FK), `quantity`, `sugarlevel`, `icelevel`, `priceatorder`, `subtotal`.
...
*   `type` (varchar): 'Khuyến mãi', 'Tuyển dụng', 'Tin tức'.
